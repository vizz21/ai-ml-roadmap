# ai-ml-roadmap
**Self-directed AI-ML study alongside Masters in AI**


## Progress
- [x] Week 1 - Python OOP, Generators, Decorators, File I/O, NumPy 
- [x] Week 2 - Pandas + EDA
- [x] Week 3 - Supervised ML
- [x] Week 4 - Ensemble Methods
- [ ] Week 5 - Feature Engineering
- [ ] Week 6 - Unsupervised ML

## Projects
- Week 1: NumPy Utility Library (mean, variance, std, covariance, correlation, linear regression)
- Week 2:  [Titanic EDA](https://github.com/vizz21/ai-ml-roadmap/blob/main/phase1-core-ml/week2-pandas-eda-visualisation/Week_2project_titanic_eda.ipynb): 10+ charts, 5 insights, blog post structure
- Week 3:  [Titanic classifier](https://github.com/vizz21/ai-ml-roadmap/blob/main/phase1-core-ml/Week3-SupervisedML-Regression%26Classification/Week3_Project.ipynb): Full ML pipeline: data cleaning → feature engineering → Logistic Regression + Decision Tree. Evaluated with accuracy, precision, recall, F1, AUC. Kaggle submission: 0.7655.
